...
class MarketData:
    ...
    def construct_next_esi_endpoint(self, idx):
        ...
        return  f'{ESI_ENDPOINT}/latest/markets/{self.region}' \
                f'/orders/?datasource=tranquility&order_type={self.order_type}' \
                f'&page={idx}'
    ...
    async def execute_requests(self):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if len(self.station_ids) > 0:
                self.orders = [
                    item for item in self.orders if item['location_id'] in self.station_ids
                ]

        end_time = time.time() - start_time
        print(
            f'--- {end_time}s ({self.region} = {len(self.orders)} {self.order_type} orders) ---'
        )
